...
import { BUFFER_DONUT_COLOR, GROUP_DONUT_COLORS, moneyColor } from '../theme'
...
const GROUP_ORDER: { key: keyof BudgetSummary['groups']; label: string; color: string }[] = [
  { key: 'needs', label: 'Needs', color: GROUP_DONUT_COLORS.needs },
  { key: 'wants', label: 'Wants', color: GROUP_DONUT_COLORS.wants },
  { key: 'discretionary', label: 'Discretionary', color: GROUP_DONUT_COLORS.discretionary },
  { key: 'temporary', label: 'Temporary', color: GROUP_DONUT_COLORS.temporary },
  { key: 'savings', label: 'Savings', color: GROUP_DONUT_COLORS.savings },
  { key: 'investments', label: 'Investments', color: GROUP_DONUT_COLORS.investments },
]
...
const BUFFER_COLOR = BUFFER_DONUT_COLOR
 ...
 * `signed` is set the value takes its semantic money colour (green/red).
 */
function TotalTile({
  label,
  cents,
...
function RunningRow({
  ...
  const moneyProps = signed ? { c: moneyColor(amounts.fortnightlyCents) } : {}
  return (
    <Table.Tr bg="var(--mantine-primary-color-light)">
      <Table.Th scope="row">{label}</Table.Th>
      <Table.Td fw={700} {...moneyProps}>
        {formatCents(amounts.fortnightlyCents)}
      </Table.Td>
      <Table.Td fw={700} {...moneyProps}>
        {formatCents(amounts.annualCents)}
