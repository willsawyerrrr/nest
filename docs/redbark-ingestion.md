...
developers rather than end-consumer budgeting. It sits on top of the
regulated Open Banking rails rather than screen-scraping:
...
- **Coverage.** 100+ AU/NZ institutions, including the Big Four (CommBank,
  Westpac, ANZ, NAB), Macquarie, Interactive Brokers, and Up Bank itself.
- **API shape.** Bearer-token REST API, eight endpoints across transactions,
  balances, accounts, connections, holdings, and trades. A transaction record
...
- No other schema change. `public.accounts`, `public.account_balance`, and
  `public.transactions` take a Redbark-sourced row exactly as they take an Up
  one.
...
developer-API integration. It lands after the Up ledger sync foundation, since
