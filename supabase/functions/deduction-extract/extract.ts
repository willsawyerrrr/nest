 ...
 *
 * **Extraction never writes a deduction anywhere.** It returns the fields it
 * read; the client pre-fills the add-deduction form, the member confirms, and
 * the member's own save is what persists — there is deliberately no write path
...
/** An object read from the receipts bucket. */
...
export interface ExtractionBody extends DeductionExtraction {
  /** The pinned model that read the receipt, so a stale client can tell. */
  model: string
}
...
export async function runExtract(rawPath: unknown, deps: ExtractDeps): Promise<FlowResult> {
  ...
  if (!object) {
    return { status: 404, body: { error: 'That receipt file could not be found.' } }
  }
  if (object.bytes.length === 0) {
    return { status: 400, body: { error: 'That receipt file is empty.' } }
  }
  ...
  const body: ExtractionBody = { model: DEDUCTION_MODEL, ...toExtraction(result.fields) }
...
/** What a failure no retry can fix says: the details are typed by hand instead. */
const UNREADABLE_MESSAGE = 'The receipt could not be read. Enter the details by hand.'
...
function modelFailure(result: Extract<ModelResult, { ok: false }>): FlowResult {
  if (result.failure === 'no_credit') {
    ...
    // the member's side: reading is switched off pending an operator, not
    // broken and not their file. Retrying cannot help, so it is not offered.
    return {
      status: 503,
      body: {
        error:
  ...
  if (result.failure === 'key_rejected') {
    // The third `503`, for the third thing only an operator can fix.
    return {
      status: 503,
      body: {
        error:
  ...
  if (result.failure === 'timeout') {
    return {
      status: 504,
      body: { error: 'Reading the receipt took too long. Try again, or enter it by hand.' },
    }
  }
  ...
  if (result.failure === 'refused') {
    ...
    return {
      status: 422,
      body: { error: 'That file could not be read. Enter the details by hand.' },
    }
  }
  ...
  if (result.status === 429) {
    return {
      status: 429,
      body: { error: 'Reading receipts is rate limited right now. Try again shortly.' },
    }
  }
  ...
  return {
    ...
    body: {
      error: transient
        ? 'The receipt could not be read right now. Try again, or enter it by hand.'
        : UNREADABLE_MESSAGE,
    },
  }
}
