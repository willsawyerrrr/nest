...
Deno.test('normalisePath rejects an absent, rooted, or traversing path', () => {
  ...
  assertEquals(normalisePath('receipt.pdf'), '') // No household prefix.
  assertEquals(normalisePath(`/${PATH}`), '')
  assertEquals(normalisePath(`${HOUSEHOLD}/../hh-2/receipt.pdf`), '')
...
Deno.test('runExtract returns the fields as integer cents alongside what was read', async () => {
  ...
  assertEquals((body.fields as Record<string, unknown>).amount_cents, 12450)
  assertEquals((body.fields as Record<string, unknown>).description, 'Officeworks')
  assertEquals((body.fields as Record<string, unknown>).deduction_date, '2026-07-06')
  assertEquals((body.text as Record<string, unknown>).amount, '124.50')
...
Deno.test('runExtract succeeds on a partial receipt, reporting every null field', async () => {
  ...
  assertEquals((body.missing as string[]).length, 3)
})
...
Deno.test('runExtract turns malformed model output into a clean error', async () => {
  ...
  assertEquals(
    ...
    'The receipt could not be read. Enter the details by hand.',
  )
  assertEquals(d.calls, ['resolveHousehold', 'apiKey', 'downloadObject', 'extract'])
})
...
Deno.test('runExtract reports a model refusal as a content problem, not a server error', async () => {
  ...
  assertEquals(
    (result.body as Record<string, unknown>).error,
    'That file could not be read. Enter the details by hand.',
  )
})
...
Deno.test('runExtract maps a model timeout to a gateway timeout', async () => {
  ...
  assertEquals(
    (result.body as Record<string, unknown>).error,
    'Reading the receipt took too long. Try again, or enter it by hand.',
  )
})
